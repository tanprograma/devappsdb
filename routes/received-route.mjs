...
router.post("/", async (req, res) => {
  const product = await Model.ReceivedModel.create(req.body);
...
router.get("/", async (req, res) => {
  const result = await Model.ReceivedModel.find().populate({
    path: "commodities",
...
//   const products = await.ReceivedModel.readAllOfKind(req.body);
//   if (!products.isSuccessful) return products.error.message;
//   // map objects to lodash objects and send
//   result = products.result.map((item) => {
//     return _.pick(item, ["type", "product", "unit", "quantity"]);
...
//   const product = await.ReceivedModel.readOne(req.body);
//   if (!product.isSuccessful) return product.error.message;
//   // map objects to lodash objects and send
...
//     "unit-3": unit,
//   } = req.body;
//   await.ReceivedModel.updateQuantity({ product, quantity });
//   const results = await.ReceivedModel.updateUnit({ product, unit });
...
//   const results = await.ReceivedModel.updateQuantity({ product, quantity });
//   if (!results.isSuccessful) return results.error.message;
//   // map objects to lodash objects and send
//   const result = _.pick(results.result, [
//     "type",
...
//   const product = await.ReceivedModel.deleteOne(req.body);
//   if (!product.isSuccessful) return product.error.message;
//   res.send(product.result);
// });
// router.post("/delete-all-of-kind", async (req, res) => {
//   const products = await.ReceivedModel.deleteAllOfKind(req.body);
//   if (!products.isSuccessful) return products.error.message;
//   res.send(products.result);
// });
// router.post("/delete-all", async (req, res) => {
//   const products = await.ReceivedModel.deleteAll();
//   if (!products.isSuccessful) return products.error.message;
//   res.send(products.result);
// });
